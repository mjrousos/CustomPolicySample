...
{
    internal class ProjectPolicyProvider : IAuthorizationPolicyProvider
    {
        const string POLICY_PREFIX = "Project";
        public DefaultAuthorizationPolicyProvider FallbackPolicyProvider { get; }

        public ProjectPolicyProvider(IOptions<AuthorizationOptions> options)
        {
        ...
        // Policies are looked up by string name, so expect 'parameters' (like ProjectName)
        // to be embedded in the policy names. This is abstracted away from developers
        // by the more strongly-typed attributes derived from AuthorizeAttribute
        // (like [ProjectAuthorize] in this sample)
        public Task<AuthorizationPolicy> GetPolicyAsync(string policyName)
        {
            if (policyName.StartsWith(POLICY_PREFIX, StringComparison.OrdinalIgnoreCase))
            {
                var policy = new AuthorizationPolicyBuilder();
                policy.AddRequirements(new ProjectRequirement(policyName.Substring(POLICY_PREFIX.Length)));
